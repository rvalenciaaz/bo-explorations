...
def main():
    ...
    total_combinations = len(batch_sizes) * len(initial_samples_range)
    total_runs = total_combinations * runs_per_combination
    
    # Initialize the outer progress bar
    with tqdm(total=total_combinations, desc="Batch & Initial Sample Combinations", unit="combo") as pbar_outer:
        ...
        for batch_size, initial_samples in itertools.product(batch_sizes, initial_samples_range):
            # Initialize list to store iteration counts for current combination
            iteration_counts = []
            
            # Initialize the inner progress bar for triplicate runs
            for run in range(1, runs_per_combination + 1):
                run_desc = f"Batch: {batch_size}, Init: {initial_samples}, Run: {run}"
                with tqdm(total=1, desc=run_desc, leave=False, position=1, unit="run") as pbar_inner:
                    iterations = bo_run(
            ...
            # Calculate mean iterations for the current combination
            ...
            pbar_outer.set_postfix({
                ...
                'Current Initial Samples': initial_samples,
                'Mean Iterations': f"{mean_iterations:.2f}"
            })
    
    # Create a DataFrame and save to CSV
    df_results = pd.DataFrame(results)
    df_results.to_csv("mean_iterations.csv", index=False)
    print("\nAll results have been saved to 'mean_iterations.csv'.")
