#doesnt work because of gpu/cuda interaction 
import torch
...
# Wrapper function to run each combination in parallel
def run_combination(batch_size, initial_samples, runs_per_combination, bounds, max_y, device, dtype, max_iterations=100):
    results = []
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        ...
        # As the futures complete, append the results
        for future in as_completed(futures):
            all_results.extend(future.result())
    
    # Create a DataFrame and save to CSV
    df_results = pd.DataFrame(all_results)
    df_results.to_csv("iterations_per_run_parallel.csv", index=False)
    print("\nAll results have been saved to 'iterations_per_run_parallel.csv'.")
